...
<body>
      ...
      <div class="top-line">
            ...
            <div class="line-search">
              <form action="search" method="get">
                  ...
                  <div style="float:right;margin-top:-30px;margin-right:194.5px;"><input type="button" onclick="search()" style="border:0px;width:30px;height:30px;background:url(/hon/image/search.png);background-color:#ffffff;background-size:100%;background-repeat:no-repeat;" /></div>
              </form>
            </div>
      </div>
    </div>
    ...
    <div id="mainpage-mid" style="margin-top:-450px;margin-left:-30px;width:1000px;">
		<div id="content" style="margin-top:500px">
			<c:forEach var="article" items="${pageObjective.articles}">
					<div style="margin-top:-180px">
						...
						<div style="width:1000px;">
							<div style="margin-left:270px;margin-top:-85px;"><a id="wrap" href="user_article.jsp?aid=${article.id}">${article.title}</a></div>
						...
						<div style="margin-left:820px;margin-top:20px;">
							<input type="button" style="border:0px;background-color:#ffffff" onclick="agreeArticle('${sessionScope.user.id}',${article.id},${article.agreementnum})" value="赞(${article.agreementnum})">
							<input type="button" style="border:0px;background-color:#ffffff" value="评论(${article.commentnum})" onclick="comment('${sessionScope.user.id}','com${article.id}')">
							<input type="button" style="border:0px;background-color:#ffffff" value="转发(${article.transpondnum})" onclick="transpond('${sessionScope.user.id}','tran${article.id}')">
							<div id="com${article.id}" style="margin-left:-270px;margin-top:15px;display:none;"><input type="text" id="cc${article.id}" style="width:400px;"><input type="button" value="评论" id="submitComment" onclick="commentSubmit('${sessionScope.user.id}',${article.commentnum},${article.id},'cc${article.id}')"></div>
							<div id="tran${article.id}" style="margin-left:-270px;margin-top:15px;display:none;"><input type="text" id="tc${article.id}" style="width:400px;"><input type="button" value="转发" id="submitTranspond" onclick="transpondSubmit('${sessionScope.user.id}',${article.transpondnum},${article.id},'tc${article.id}')"></div>
						</div>
    ...
    <div id="mainpage-asize" style="position:fixed;left:1100px;top:50px;width:300px">
			<c:if test="${sessionScope.user.id!=null}">
				<sql:query dataSource="${snapshot}" var="result">
					select user_pic,user_name from user where user_id = ?
					<sql:param value="${sessionScope.user.id}" />
				</sql:query>
				<div style="background-color:#fff6ed;margin-top:60px">
					<c:forEach var="row" items="${result.rows}">
						<div style="margin-left:10px;"><img src="${row.user_pic}" style="width:60px;height:60px"></div>
						<div style="margin-left:80px;margin-top:-60px;font-size:25px;">${row.user_name}</div>
						<div style="margin-left:80px;margin-top:10px;"><a href="user_homepage.jsp">个人中心</a></div>
					</c:forEach>
				</div>
			</c:if>
			<c:if test="${sessionScope.user.id==null}">
